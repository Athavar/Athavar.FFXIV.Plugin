...
internal sealed class Encounter : BaseEncounter<Combatant>
{
    ...
    public Encounter(string territoryName, uint territory, DateTime start)
    ...
    public Combatant? GetCombatant(uint objectId)
    {
        ...
        else if (gameObject is IBattleNpc battleNpc)
        {
            ...
            combatant = new Combatant(this, oid, battleNpc.BaseId)
            {
                ...
                Name = name,
                Name_First = name,
                Job = battleNpc.BattleNpcKind == BattleNpcSubKind.Buddy ? Job.Chocobo : (Job)battleNpc.ClassJob.RowId,
    ...
    public unsafe void UpdateParty(IDalamudServices services)
    {
        ...
        this.Title = $"{combatants.Where(c => c.Kind == BattleNpcSubKind.Combatant).MaxBy(c => c.DamageTotal)?.Name}";
        ...
        foreach (var chocobo in combatants.Where(c => c.Kind == BattleNpcSubKind.Buddy))
        {
    ...
    public override bool IsValid() => this.Start != DateTime.MinValue && this.AllyCombatants.Any() && this.Combatants.Any(c => c.Kind == BattleNpcSubKind.Combatant && c.IsActive());
