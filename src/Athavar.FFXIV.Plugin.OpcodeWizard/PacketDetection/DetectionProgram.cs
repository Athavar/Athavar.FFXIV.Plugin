...
internal class DetectionProgram : IDisposable
{
    ...
    public DetectionProgram(IOpcodeManager opcodeManager, ScannerRegistry scannerRegistry)
    {
        this.opcodeManager = opcodeManager;
        this.scannerRegistry = scannerRegistry;

        // this.dalamudServices.GameNetwork.NetworkMessage += this.OnMessage;
        this.pq = new Queue<Packet>();
    }

    public bool IsRunning => !this.stopped;

    public int ScannerIndex { get; protected set; }

    public string CurrentTutorial { get; protected set; } = string.Empty;
    ...
    private Task RunScanner(Scanner scanner, string[] parameters)
        => Task.Run(() =>
        {
            try
            {
                var opcode = PacketScanner.Scan(this.pq, scanner, parameters, ref this.skipped, ref this.stopped, ref this.Debug, this.opcodeManager.Opcodes);
