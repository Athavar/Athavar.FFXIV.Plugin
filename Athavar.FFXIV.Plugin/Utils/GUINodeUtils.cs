...
{
    /* ################################
    Source: https://github.com/MgAl2O4/FFTriadBuddyDalamud/blob/main/utils/GUINodeUtils.cs
    ################################### */
    ...
    {
        public static unsafe AtkResNode*[]? FindNode(AtkResNode* rootNode, NodeType type)
        {
            var listAddr = new List<ulong>();
            var stackAddr = new Stack<ulong>();
            stackAddr.Push((ulong)rootNode);

            while (stackAddr.TryPop(out var next))
            {
                var node = (AtkResNode*)next;

                var childs = GetImmediateChildNodes(node);

                if (childs is not null)
                    foreach (ulong child in childs)
                    {
                        stackAddr.Push(child);
                    }
        ...
        private static unsafe AtkResNode* ConvertToNodeArr(ulong addr)
        {
            return (AtkResNode*)addr;
        }
        ...
        private static unsafe bool RecursiveAppendParsableChildNodes(AtkResNode* node, int depth, int childIdx, List<ParsableNode> list, string debugPath)
        {
            ...
            if (node != null)
            {
                ...
                if ((hasParsableChildNodes || hasContent) && content is not null)
                {
