...
namespace Athavar.FFXIV.Plugin.Module.Yes
{
    ...
    internal abstract class OnSetupSelectListFeature : OnSetupFeature, IDisposable
    {
        ...
        {
            var millisSinceLastEscape = (DateTime.Now - YesService.Module!.EscapeLastPressed).TotalMilliseconds;
            ...
            var nodes = YesService.Configuration.GetAllNodes().OfType<ListEntryNode>();
        ...
        private unsafe byte OnItemSelectedDetour(IntPtr popupMenu, uint index, IntPtr a3, IntPtr a4)
        {
            try
            {
                ...
                if (index < popupMenuPtr->EntryCount)
                {
                    ...
                    var entryText = YesService.Module!.LastSeenListSelection = entryPtr != null
                        ? YesModule.GetSeStringText(entryPtr)
                        : string.Empty;

                    var target = DalamudBinding.TargetManager.Target;
                    var targetName = YesService.Module.LastSeenListTarget = target != null
                        ? YesModule.GetSeStringText(target.Name)
                        : string.Empty;
        ...
        private unsafe string?[] GetEntryTexts(PopupMenu* popupMenu)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                entryTexts[i] = textPtr != null
                    ? YesModule.GetSeStringText(textPtr)
                    : null;
            }
